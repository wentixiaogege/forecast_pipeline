...
with tqdm(total=len(cats), desc='  Encoding', unit='cols') as pbar:
    for col, cat in enumerate(cats):
        value_counts = dict(list(zip(*np.unique(train_cat[:, col], return_counts=True))))
        ...
        for val in value_counts:
            if value_counts[val] >= min_freq:
                ...
                train_cat_enc.append(sp.csr_matrix((train_cat[:, col] == val).astype(np.uint8).reshape((train_cat.shape[0], 1)))) # 判断是不是这个数
...
train_cat_enc = sp.hstack(train_cat_enc, format='csr') # 按照列拼接到一起
test_cat_enc = sp.hstack(test_cat_enc, format='csr') # 按照列拼接到一起
